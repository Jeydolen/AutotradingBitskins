SELECT * FROM
	( 	
		...
		(	
			SELECT id FROM bitskins_csgo.skin WHERE skin_set = 5 AND skin_rarity = 3 AND item_state = 4
        )
	) SQ_A
INNER JOIN
	( 	
		...
		(	
			SELECT id FROM bitskins_csgo.skin WHERE skin_set = 5 AND skin_rarity = 4 AND item_state = 4
        )
	) SQ_B
