SELECT * FROM
	( 	
		...
		(	
			SELECT id FROM bitskins_csgo.skin WHERE skin_set = 5 AND skin_rarity = 4 AND item_state = 4 AND has_StatTrak = 1
        )
	) SQ_A
INNER JOIN
	( 	
		...
		(	
			SELECT id FROM bitskins_csgo.skin WHERE skin_set = 5 AND skin_rarity = 5 AND item_state = 4 AND has_StatTrak = 1
        )
	) SQ_B
