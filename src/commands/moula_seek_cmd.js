...
class MoulaSeekCmd extends Command
{
    ...
    computeProfit ( trade_up )
    {
        ...
        if ( upgrades_count != 0 )
        {
            Array.from ( trade_up.target_siblings.values() ).map(
               (target_sibling) => { total_profit += target_sibling.price }
            )
        } */
        return "\n Upgrades: " + upgrades_count + ": investment = " + total_investment.toFixed(2);
    }
